...
console.log($scope.budgetRow);
        ...
        var getBudgetData = function (flag){
            ...
            $http({
                method  : 'GET',
                url     : 'controllers/getBudget.php'+param,
                data : param
            }).success(function(data) {
                console.log(flag);
                console.log(data);
                if(!$.isNumeric(flag)){
                    $scope.getTotal = getTotal(data.total);

                    $scope.resultData = data.records;
                }else{
                    var rec = data.records;
                     $scope.budgetRow = rec;
                    $scope.name = rec.name;
                     $scope.kindsDataModel = rec.type;
                }
